...
namespace ShopForPerts.Controllers
{
    ...
    {
        private readonly IAllSeller _allProduts;
        
        private readonly AppDB _context;

        public SellerController(IAllSeller iAllProducts, AppDB dBContent)
        {
        ...
        public IActionResult AddOrEdit(int id = 0)
        {
            if (id == 0)
                ...
                return View(_context.Seller.Find(id));
        }

        [HttpPost]
        [ValidateAntiForgeryToken]

        public async Task<IActionResult> AddOrEdit([Bind("Id,LastName,FirstName,MiddleName,BirthDate,Phone,Email,Password,isWork,yearWork")] Seller transaction)
        {
        ...
        {
            var transaction = await _context.Seller.FindAsync(id);
            _context.Seller.Remove(transaction);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
